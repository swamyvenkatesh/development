...
def cartModelsDataCreation(request, fare, fare_class, passengers_num, prodDetails, passengers=[]):
    ...
    cartProduct = CartProducts.objects.create(
        cart_id=addCart, service=str(fare_class), product_name=product_name, product_id=1,
        status=0, netprice=Money(fare, 'GBP'), passengers_num=int(passengers_num),
        start_date=dept_date, Rule='Non-Refundable', fare=Money(fare, 'GBP'), settlementprice=Money(fare, 'GBP'))
    ...
    for detail in prodDetails['multiple_segments']:
        ...
        prod_detailsList.append(
            CartProductDetails(cart_product_id=cartProduct, from_station=str(detail['dept_station']),
    ...
    for passsenger in passengers:
        for i in passsenger:
            aldetails.append(CartProductPassengers(cart_product_id=cartProduct, first_name=i['first_name'],
