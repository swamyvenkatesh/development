...
def ticket_view(request):
    ...
    if request.method == "POST":
        ...
        print fare, fare_class, train_no, heading, no_of_adults, no_of_childs, "---"
                        ...
                        if result.get('fares'):
                            for ind1,fare in enumerate(result['fares']):
                                ...
                                if fare['product_name'] not in dd:                                   
                                    dict1 = {}
                                    dict1[fare['class']] = float(fare['fare'])
                                    dd[fare['product_name']] = dict1
                                else:
                                    name = dd.get(fare['product_name'])
                                    name[fare['class']] = float(fare['fare'])
                                # print "inner loop"
                                # print dd                        
                        ddd[main_index] = dd               
                        dd = {}
