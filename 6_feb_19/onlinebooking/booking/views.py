...
def checkinfodeatil(request):
    ...
    if request.method == "POST":
        ...
        delete_item = CartProductDetails.objects.filter(id=remove_cart_id).delete()
        # print "After Removing"
        # print delete_item

        cart = Cart.objects.get(sessionId=session_id)
        # print cart
        cartProduct = CartProducts.objects.get(cart_id=cart.id)
        # print cartProduct, "^^^"

        cart_products = CartProductDetails.objects.filter(cart_product_id=cartProduct.cart_id).all()
        # print cart_products , "))"

        productDetails = cart_products
        # print "dfsdfsd"*20
        # print productDetails
        pro = []
        for i in productDetails:
...
def cartModelsDataCreation(request, fare,fare_class,passengers_num, prodDetails, passengers=[]):
    ...
    cart_details = Cart.objects.filter(sessionId=session_id)
    # cartId = 0
    if cart_details:
        print "If cart available"
        ...
        cartProduct = CartProducts.objects.filter(cart_id=cartId1)
        # If cart product is avaliable
        if cartProduct:
            cartProductId1 = 0
            for i in cartProduct:
                cartProductId1 = i.cart_id

            # cart product details creation
            productDetails = CartProductDetails.objects.create(
                cart_product_id=cartProductId1,from_station=str(prodDetails['dept_station']),
            ...
            for passsenger in passengers:
                for i in passsenger:                
                    aldetails.append(CartProductPassengers(cart_product_id=cartProductId1,first_name=i['first_name'],
                    last_name=i['secondname'],dob=datetime.now(),nationality=i['nationality'], passport=i['passport']))                
            # print "*"*25
            # print aldetails
            CartProductPassengers.objects.bulk_create(aldetails)
            print "cart product passsengers  created"

            productAllDetails = CartProductDetails.objects.filter(cart_product_id=cartProductId1).all()
            ...
            # cartProductId = cartProduct.cart_id
            print "cart products created" 

            # Cart Product Details Creation


            productDetails = CartProductDetails.objects.create(
                cart_product_id=cartProduct.cart_id,from_station=str(prodDetails['dept_station']),
                to_station=str(prodDetails['arr_station']),from_code=str(prodDetails['dept_station_code']),to_code=str(prodDetails['arr_station_code']),
                departure_date=depDateTime,arrival_date=arrDateTime, train=str(prodDetails['train']),train_category=str(fare_class),
                netprice=Money(fare, 'GBP'),passengers_num=int(passengers_num))
            ...
            for passsenger in passengers:
                for i in passsenger:                
                    aldetails.append(CartProductPassengers(cart_product_id=cartProduct.cart_id,first_name=i['first_name'],
                    last_name=i['secondname'],dob=datetime.now(),nationality=i['nationality'], passport=i['passport']))                
            # print "*"  
            CartProductPassengers.objects.bulk_create(aldetails)
            print "cart product passsengers  created"

            productAllDetails = CartProductDetails.objects.filter(cart_product_id=cartProduct.cart_id).all()
        ...
        cartProduct = CartProducts.objects.filter(cart_id=cart_details.id)
        if cartProduct:
            cartProductId2 = 0
            for i in cartProduct:
                cartProductId2 = i.cart_id

            # cart product details creation
            productDetails = CartProductDetails.objects.create(
                cart_product_id=cartProductId2,from_station=str(prodDetails['dept_station']),
                to_station=str(prodDetails['arr_station']),from_code=str(prodDetails['dept_station_code']),to_code=str(prodDetails['arr_station_code']),
                departure_date=depDateTime,arrival_date=arrDateTime, train=str(prodDetails['train']),train_category=str(fare_class),
                netprice=Money(fare, 'GBP'),passengers_num=int(passengers_num))
            ...
            for passsenger in passengers:
                for i in passsenger:                
                    aldetails.append(CartProductPassengers(cart_product_id=cartProductId2,first_name=i['first_name'],
                    last_name=i['secondname'],dob=datetime.now(),nationality=i['nationality'], passport=i['passport']))                

            CartProductPassengers.objects.bulk_create(aldetails)
            print "cart product passsengers  created"

            productAllDetails = CartProductDetails.objects.filter(cart_product_id=cartProductId2).all()            

            return [productAllDetails]
        else:
            cartProduct = CartProducts.objects.create(cart_id=cart_details.id,product_name=str(fare_class),product_id=1,
                status=0,netprice=Money(fare, 'GBP'),passengers_num=int(passengers_num))
            # cartProductId = cartProduct.cart_id
            print "cart products created" 
